...
import { buildGoogleFormAppsScript } from "@/lib/google-form-apps-script";
...
export function AppointmentCaptureLinkManager({
  ...
  const googleFormScript = useMemo(
    () =>
      createdUrl ? buildGoogleFormAppsScript(createdUrl, createdPasscode) : "",
    [createdPasscode, createdUrl],
  );
  ...
  async function action(id: string, kind: "rotate" | "revoke") {
    ...
    try {
      const response = await fetch(
        `/api/appointment-capture-links/${id}/${kind}`,
        { method: "POST" },
      );
      const result = await response.json().catch(() => ({}));
      if (!response.ok)
      ...
      setLinks((current) =>
        current.map((item) =>
          item.id === id
            ? { ...result.item, publicUrl: publicUrl || item.publicUrl }
            : item,
        ),
      );
      if (publicUrl) {
    ...
    <div className="grid gap-6 lg:grid-cols-[360px_1fr]">
      <div className="space-y-6">
        ...
        {googleFormScript ? (
          <section className="card p-5">
            <h2 className="font-semibold">Googleフォーム連携</h2>
            <p className="mt-1 text-sm text-slate-500">
              フォームのApps Scriptへ設定するコードです。
            </p>
            <textarea
              className="text-field mt-4 min-h-48 font-mono text-xs"
              value={googleFormScript}
              readOnly
              aria-label="Googleフォーム連携スクリプト"
            />
            <button
              ...
              onClick={async () => {
                ...
                setMessage("Googleフォーム用スクリプトをコピーしました。");
              }}
            >
              スクリプトをコピー
            </button>
      ...
      <section className="card overflow-hidden">
        <div className="border-b border-line px-5 py-4">
          ...
          <p className="mt-1 text-sm text-slate-500">
            公開入力URLは{" "}
            <code className="rounded bg-slate-100 px-1">/a/[token]</code>{" "}
            です。tokenはDBに保存せず、作成時または再発行時だけ表示します。
          </p>
        </div>
            ...
            <div key={link.id} className="px-5 py-4">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="text-xs text-slate-400">
                  送信 {link.submissionCount}
                  {link.maxSubmissions ? ` / ${link.maxSubmissions}` : ""}
                </span>
              </div>
              <p className="mt-1 text-sm text-slate-500">
                {businessUnits.find((unit) => unit.id === link.businessUnitId)
                  ?.name ?? "事業部不明"}{" "}
                /{" "}
                {isUsers.find(
                  (user) => user.id === link.creditedAppointmentSetterId,
                )?.name ?? "担当者不明"}
              </p>
              <div className="mt-3 rounded-lg bg-slate-50 p-3">
                ...
                {link.publicUrl ? (
                  <div className="mt-2 flex flex-wrap items-center gap-2">
                    <a
                      className="break-all text-sm font-bold text-brand-700 underline"
                      href={link.publicUrl}
                      target="_blank"
                      rel="noreferrer"
                    >
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                <button
                  ...
                  type="button"
                  disabled={pending}
                  onClick={() => action(link.id, "rotate")}
                >
                  リンク再発行
                </button>
                <button
                  className="secondary-button border-red-200 text-red-600"
                  type="button"
                  disabled={pending || link.status !== "ACTIVE"}
                  onClick={() => action(link.id, "revoke")}
                >
                  失効
                </button>
              </div>
